...
namespace App\Reports\Inspection;
...
class AttainmentReport
{
    ...
    public function __construct(Request $request,array $schools_ids)
    {
        $this->request = $request;


        // If specific schools are selected in request
        $this->schools = School::whereIn('id', $schools_ids)->get();

        // Initialize rounds based on school curriculum type
        if ($this->schools->first() && $this->schools->first()->type == 'Indian') {
    ...
    {
        $title = re('The Attainment Report') . ' - ' . re('Inspection Report');
        ...
        // Define the rounds order
        $ordered_rounds = [];
        foreach ($this->rounds_key as $round_key) {
            $ordered_rounds[] = $this->rounds[$round_key];
        }

        // Process data for each school
        foreach ($this->schools as $school) {
            $arab_grades = [];
            $non_arab_grades = [];

            // Apply filters based on student type
            if ($student_type == 1 || $student_type == 0) {
                $arab_grades = $this->processSubject($school, 1, $subjects, $grades, $year, $sections, $include_sen, $include_g_t, $ordered_rounds);
            }

            if ($student_type == 2 || $student_type == 0) {
                $non_arab_grades = $this->processSubject($school, 2, $subjects, $grades, $year, $sections, $include_sen, $include_g_t, $ordered_rounds);
            }

            // Only add schools that have data
            if (!empty($arab_grades) || !empty($non_arab_grades)) {
                $schoolsData[$school->id] = [
                    'school' => $school,
                    'arab_grades' => $arab_grades,
                    'non_arab_grades' => $non_arab_grades
                ];
            }
        }
        ...
        // Return appropriate view based on summary parameter
        if ($this->request->get('summary', false)) {
            $title = re('Attainment Summary') . ' - ' . re('Inspection Report');
            return view('general.reports.attainment.new.report-summary', compact(
                'title', 'schoolsData', 'rounds', 'grades', 'year',
                'sections', 'include_g_t', 'include_sen', 'subjects', 'student_type'
            ));
        }

        return view('general.reports.attainment.new.report', compact(
            'title', 'schoolsData', 'rounds', 'grades', 'year',
            'sections', 'include_g_t', 'include_sen', 'subjects', 'student_type'
        ));
    }

    private function processSubject($school, $is_arabic, $subjects, $grades, $year, $sections, $include_sen, $include_g_t, $rounds)
    {
        $result = [];
