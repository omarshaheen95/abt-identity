...
class SchoolCombinedProgressReport
{
    public $school;
    public $request;
    public $start;
    ...
    public function report()
    {
        ...
        //September
        $september_terms = Term::query()->whereHas('level', function (Builder $query) use ($grades, $custom_year, $type) {
        ...
        //February
        $february_terms = Term::query()->whereHas('level', function (Builder $query) use ($grades, $year, $type) {
            $query->whereIn('grade', $grades)
                ->when($type != 2, function (Builder $query) use ($type) {
                    $query->where('arab', $type);
                })
        ...
        //May
        $may_terms = Term::query()->whereHas('level', function (Builder $query) use ($grades, $year, $type) {
            $query->whereIn('grade', $grades)
                ->when($type != 2, function (Builder $query) use ($type) {
                    $query->where('arab', $type);
                })
        ...
        if ($student_terms->count()) {
            $arab_pages[0] = (object)[
                'title' => "Combined Grade $first_grade - $last_grade $sub_title Progress - $year->name",
                'septProgress' => $septProgressData,
                'febProgressData' => $febProgressData,
                'mayProgressData' => $mayProgressData,
                'summary_title' => "Summary Combined Grade $first_grade - $last_grade $sub_title Progress - $year->name",
