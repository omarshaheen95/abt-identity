            ...
            ROUND(
                COUNT(CASE WHEN sts.mark >=
                    CASE
                        ...
                        WHEN question_standards.mark = 3 THEN 2
                        WHEN question_standards.mark = 5 THEN 3
                        ELSE 1
                    END
...
// Step 3: Query standards with relationships and additional conditions
        $standards = QuestionStandard::query()
            ...
            ->with(['studentTermStandards' => function ($query) use ($studentId) {
                $query->where('student_id', $studentId);
            }])
            ->whereHas('question', function (Builder $query) use ($id_terms, $skills) {
                $query->whereIn('term_id', $id_terms);
                $query->whereIn('type', $skills);
            })
            ->get();
        $names = array();
        $marks = array();
        ...
        return view('general.reports.student_report', compact('standards', 'names', 'full_marks', 'full_data_term', 'data_term', 'student', 'marks', 'terms', 'school'))->render();
    }
