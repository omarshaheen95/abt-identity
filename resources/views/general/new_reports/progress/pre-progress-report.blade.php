...
@section('content')
    <form class="form" id="form_data" action="{{route(getGuard().'.report.progress-report')}}" method="get">
        ...
        <div class="d-flex justify-content-end gap-3">
            <button type="button" class="btn btn-lg get-report gradient-btn gradient-btn-primary"
                    data-route="{{route(getGuard().'.report.progress-report')}}">
...
@section('script')
    <script>
        $(document).ready(function () {
            const ERROR_TITLE = '{{t('Error')}}';
            const ERROR_MESSAGE = '{{t('Please fill in all required fields correctly.')}}';
        });
    </script>
    ...
    {!! JsValidator::formRequest(\App\Http\Requests\General\Report\ProgressReportRequest::class, '#form_data'); !!}
@endsection
